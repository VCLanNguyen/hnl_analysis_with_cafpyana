...
def add_variables(df, beam_x: float = -74.0, beam_y: float = 0.0):
    ...
    Per PFP branch (primshw / secshw / primtrk):
      (branch, obj, 'dist_vtx', '', '', '')         -- 3D distance from start to slice vertex [cm]
    ...
    if vtx_x_col in df.columns and vtx_z_col in df.columns:
        for branch, obj in [('primshw', 'shw'), ('secshw', 'shw'), ('primtrk', 'trk')]:
            sx = (branch, obj, 'start', 'x', '', '')
            sy = (branch, obj, 'start', 'y', '', '')
            sz = (branch, obj, 'start', 'z', '', '')
            out = (branch, obj, 'dist_vtx', '', '', '')
            if sx in df.columns and out not in df.columns:
                dx = df[sx].values - df[vtx_x_col].values
                dy = df[sy].values - df[vtx_y_col].values
                dz = df[sz].values - df[vtx_z_col].values
                df[out] = np.sqrt(dx**2 + dy**2 + dz**2)
