...
def get_total_cov(reco_df, reco_var, bins, normalize=False, 
                  selection_kwargs=None, projected_pot=1e20, 
                  mc_pot=mcbnb_pot, xsec_inputs=None,):
    """
    ...
    - evtrate_syst_dict (includes DetVar keys)
    ...
    sorted_df = nue.ensure_lexsorted(reco_df, axis=1)
    cv_hist = nue.get_hist1d(data=sorted_df[reco_var], weights=reco_df.flux_pot_norm, bins=bins)

    detv_syst_dict = nue.get_detvar_systs(detvar_dict, reco_var, bins, normalize=normalize, **selection_kwargs)
    evtr_syst_dict = nue.get_syst(reco_df=reco_df, reco_var=reco_var, bins=bins, normalize=normalize)
    evtrate_total_syst_dict = {**evtr_syst_dict, **detv_syst_dict}
    evtr_syst_df = nue.get_syst_df([evtr_syst_dict, detv_syst_dict], cv_hist)

    data_err = np.sqrt(nue.get_hist1d(data=sorted_df[reco_var], weights=reco_df.weights_mc, bins=bins)* (projected_pot / mc_pot))
    flux_scale = nue.integrated_flux * (projected_pot / 1e6)
    ...
    evtr_syst_df = pd.concat([evtr_syst_df, data_syst_df], ignore_index=True)
    evtrate_cov = _sum_covariances([evtr_syst_dict, detv_syst_dict], len(bins))

    if xsec_inputs is None:
        return SystematicsOutput(
            hist_cv=cv_hist,
            evtrate_cov=evtrate_cov,
            evtrate_syst_df=evtr_syst_df,
            evtrate_syst_dict=evtrate_total_syst_dict,
        )

    xsec_syst_dict = nue.get_syst(reco_df=reco_df,reco_var=reco_var,bins=bins,normalize=normalize,xsec=True,xsec_inputs=xsec_inputs)
    xsec_total_syst_dict = {**xsec_syst_dict, **detv_syst_dict}
    xsec_syst_df = nue.get_syst_df([xsec_syst_dict, detv_syst_dict], cv_hist)
