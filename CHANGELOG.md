...
## 2026-07-02 — Merge master into feature/nu26_bdt_syst (HNL analysis)

Brings the HNL/BDT branch (`bdt.py`, `timing_calibration.py`, `pyhf_helpers.py`,
`new_variables.py`) up to date with master's package reorganization and plotting/syst
rewrites, while keeping HNL analysis working on the new APIs.

### Breaking changes

**HNL categories moved into `analysis.py`, flat-list API replaced by `categories=` dicts**
`constants.py` is gone (folded into `analysis.py`, matching master's
`constants.py` → `analysis.py` / `histogram.py` → `utils.py` consolidation). The old
`signal_hnl_dict` / `signal_labels` / `signal_colors` flat-list scheme is replaced by
`signal_categories_hnl` (dict of `{value, label, color}`, same schema as master's
`signal_categories`). Pass it explicitly:

```python
# Before
from nueana import constants
constants.signal_dict = constants.signal_hnl_dict
plot_var(df, ...)  # read the globally-swapped signal_dict

# After
from nueana import signal_categories_hnl
plot_var(df, ..., categories=signal_categories_hnl)
```

`bdt.py`'s temporary HNL-legend-label swap no longer mutates `plotting.signal_labels`
(that global doesn't exist anymore); it now passes a locally-modified `categories` dict.

**`selection.py` — generic pi0 `define_signal` renamed to `define_signal_pi0`**
Master introduced its own `analysis.define_signal` (nueCC-specific scheme); keeping both
named `define_signal` would have made the effective definition depend on import order.
The pi0/HNL-scheme version (using `signal_dict_hnl`) is now `define_signal_pi0`.
`define_signal_hnl` is unchanged. The duplicate `define_ccnue_signal` / `define_generic`
(pre-reorg copies of what now lives in `analysis.py`) were removed as dead code.

### Improvements

**`plotting.py` — `plot_mc_data`, `plot_mc_hnl_data`, `plot_mc_hnl` rebuilt on master's API**
`plot_mc_data` now uses master's version directly (chi²/annotation placement, data-first
legend reordering, `annotate_sbnd`). `plot_mc_hnl_data` (MC stack + HNL step overlay +
data, with ratio panel) was rebuilt to the same standard. `plot_mc_hnl` (no data overlay,
optional Punzi FOM subplot) is unchanged.

**`syst.py` — HNL branch now uses master's vectorized xsec/syst histogram path**
The branch's own `get_syst_hists` predated master's vectorization
(`_get_xsec_hists_inner`, pre-digitized indices, `np.bincount`-based unisim/multisig/
multisim paths); master's implementation is adopted as-is.

**`io.py` — kept `correct_cosmic_weight_mevprtl_df` alongside master's `load_mc`/`load_data`**
No overlap between the two; both are additive to `load_dfs`/`get_n_split`/`print_keys`.

**`utils.py` — kept `sci_notation` alongside master's rewritten `merge_hdr`**

### Cleanup

Dropped a hardcoded personal `sys.path.append("/exp/sbnd/data/users/lnguyen/...")` from
the top of `selection.py` — should not have been committed.
